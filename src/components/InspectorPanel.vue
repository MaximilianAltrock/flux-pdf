  ...
  <aside class="w-[260px] bg-surface border-l border-border flex flex-col shrink-0 overflow-hidden">
        ...
        <div v-if="hasSelection">
          ...
          <div v-if="store.selectedCount === 1" class="mb-6">
            ...
            <div class="space-y-3">
               <!-- Source File -->
               <div>
                  <label class="text-[10px] uppercase font-bold text-text-muted mb-1 block tracking-wider">Source File</label>
                  <div class="text-xs text-text truncate bg-background border border-border rounded px-2 py-1.5" :title="getSourceName(singleSelectedPage?.sourceFileId)">
               ...
               <!-- Rotation Display -->
               <div>
                  <label class="text-[10px] uppercase font-bold text-text-muted mb-1 block tracking-wider">Rotation</label>
                  <div class="text-xs text-text bg-background border border-border rounded px-2 py-1.5 font-mono">
          ...
          <div class="space-y-4">
            <!-- Actions Grid -->
            <div>
              <span class="text-[10px] uppercase font-bold text-text-muted mb-2 block tracking-wider">Actions</span>
          ...
          <div class="bg-background border border-border rounded-lg p-3 space-y-3">
            ...
            <div class="flex justify-between items-center text-xs">
             ...
             <div class="flex justify-between items-center text-xs">
              ...
              <span class="font-mono text-text">~{{ (totalSize / 1024 / 1024).toFixed(1) }} MB</span>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="h-2/5 min-h-[220px] flex flex-col bg-background border-t border-border">
      ...
      <div class="flex-1 overflow-y-auto p-4">
        <ul class="relative space-y-0 before:absolute before:inset-y-0 before:left-[19px] before:w-px before:bg-border before:z-0">
          <li
          ...
          >
            ...
            <div
              ...
              :class="entry.isCurrent ? 'border-selection bg-selection' : (entry.isUndone ? 'border-text-muted opacity-50' : 'border-selection bg-background')"
            ></div>
            ...
            <div class="text-[10px] text-text-muted opacity-50 font-mono mt-0.5">{{ formatTime(entry.timestamp) }}</div>
          </li>

           <li v-if="historyList.length === 0" class="pl-8 py-2 text-xs text-text-muted italic opacity-50 relative z-10">
             <div class="absolute left-[15px] top-3.5 w-[9px] h-[9px] rounded-full border-2 border-border bg-background z-20"></div>
