...
function goToPrevious() {
  const prevPage = store.pages[currentIndex.value - 1]
  if (hasPrevious.value && prevPage) {
    emit('navigate', prevPage)
  }
}

function goToNext() {
  const nextPage = store.pages[currentIndex.value + 1]
      ...
      >
        ...
        <header class="flex items-center justify-between px-4 py-3 bg-background border-b border-border">
          <div class="flex items-center gap-4">
            ...
            <div class="flex items-center gap-1">
              ...
              <button
                class="px-3 py-1 text-sm text-text-muted hover:text-text hover:bg-muted/10 rounded transition-colors"
                title="Reset zoom (0)"
                @click="resetZoom"
              >
                {{ Math.round(zoom * 100) }}%
        ...
        >
          ...
          <div v-if="isLoading" class="flex flex-col items-center gap-4">
            ...
            <span class="text-text-muted">Loading preview...</span>
          </div>
        ...
        <button
          v-if="hasPrevious"
          class="absolute left-4 top-1/2 -translate-y-1/2 p-3 bg-surface/80 hover:bg-surface text-text rounded-full shadow-lg transition-colors border border-border"
          title="Previous page (←)"
          @click="goToPrevious"
        >
        ...
        <button
          v-if="hasNext"
          class="absolute right-4 top-1/2 -translate-y-1/2 p-3 bg-surface/80 hover:bg-surface text-text rounded-full shadow-lg transition-colors border border-border"
          title="Next page (→)"
          @click="goToNext"
        >
