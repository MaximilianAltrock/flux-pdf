...
function handlePageDragStart(event: DragEvent, index: number) {
  ...
  if (event.dataTransfer) {
    ...
    if (selected.length > 1) {
      const payload = { type: 'source-pages', sourceId: props.sourceId, pages: selected }
      ...
      event.dataTransfer.setData('application/x-flux-source-page', `${props.sourceId}:${pageIndex}`)
      event.dataTransfer.setData(
        'application/json',
        JSON.stringify({ type: 'source-page', sourceId: props.sourceId, pageIndex }),
      )
    }
  }
}
