...
function buildSizeMismatchRule(pages: PageReference[]): LintResult[] {
  ...
  const fix: PreflightFix = {
    ...
    label: `Resize to ${formatSizeLabel(primaryShort, primaryLong)}`,
    targets,
  }
  ...
  return [
    {
      ...
      severity: 'warning',
      message: `${outliers.length} page${outliers.length === 1 ? '' : 's'} deviate from the majority size (${formatSizeLabel(primaryShort, primaryLong)}).`,
...
function buildResolutionRules(
  ...
  for (const page of pages) {
    ...
    } else if (source.isImageSource && avgSize < LOW_QUALITY_BYTES) {
      lowQualityPageIds.push(page.id)
    }
  }
...
export function createDocumentPreflightAnalysis(store: DocumentState): ComputedRef<LintResult[]> {
  return computed<LintResult[]>(() => {
    ...
    return [
      ...buildSizeMismatchRule(pages),
      ...buildOrientationRule(pages),
      ...buildResolutionRules(pages, store.sources),
      ...buildMetadataRule(store.metadata.title),
    ]
  })
}
