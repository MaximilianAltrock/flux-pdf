...
import { getProjectThumbnailKey } from '@/domains/workspace/application/project-thumbnail.service'
...
export function createProjectAuthoringService(persistence: ProjectAuthoringPersistence) {
  ...
  async function persistProject(input: PersistProjectInput): Promise<ProjectMeta | null> {
    ...
    const thumbnailKey = getProjectThumbnailKey(firstPage)
    let thumbnail: Blob | undefined
    if (firstPage) {
      try {
        thumbnail = (await input.ensureThumbnail(existingMeta, firstPage)) ?? existingMeta.thumbnail
      } catch {
