...
export function useKeyboardShortcuts(
  ...
  function handleKeydown(e: KeyboardEvent) {
    ...
    if ((isCmd && key === SHORTCUT_KEYS.COMMAND_PALETTE) || e.key === '/') {
      e.preventDefault()
      if (!isMobile.value) {
    ...
    // Tools
    if (key === SHORTCUT_KEYS.TOOL_SELECT) {
      actions.setCurrentTool('select')
    } else if (key === SHORTCUT_KEYS.TOOL_RAZOR) {
      actions.setCurrentTool('razor')
    }

    // Undo/Redo
    if (isCmd && key === SHORTCUT_KEYS.UNDO) {
      e.preventDefault()
      if (isShift) {
        redo()
      } else {
    ...
    // Select All
    if (isCmd && key === SHORTCUT_KEYS.SELECT_ALL) {
      e.preventDefault()
    ...
    if (hasSelection.value) {
      ...
      if (isCmd && key === SHORTCUT_KEYS.LETTER_D) {
        e.preventDefault()
      ...
      if (key === SHORTCUT_KEYS.LETTER_D && !isCmd) {
        e.preventDefault()
