 ...
 */
export function mapUiBookmarksToExport(
  uiNodes: UiBookmarkNode[],
  pageIdToIndex: Map<string, number>,
): BookmarkNode[] {
  const mapNode = (n: UiBookmarkNode): BookmarkNode | null => {
    const pageIndex = pageIdToIndex.get(n.pageId)
    if (pageIndex == null) return null

    const children = (n.children ?? []).map(mapNode).filter((x): x is BookmarkNode => x !== null)
  ...
  return (uiNodes ?? []).map(mapNode).filter((x): x is BookmarkNode => x !== null)
}
 ...
 */
export async function addBookmarks(pdfDoc: PDFDocument, bookmarks: BookmarkNode[]) {
...
function countVisible(nodes: BookmarkNode[]): number {
  let count = 0
  for (const n of nodes) {
    count += 1
